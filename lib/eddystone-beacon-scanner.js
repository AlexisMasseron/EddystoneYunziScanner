...
EddystoneBeaconScanner.prototype.parseBeacon = function(peripheral) {
  ...
  var light = data2.readUInt16BE(14);
...
EddystoneBeaconScanner.prototype.parseTlmData = function(data) {
  ...
  var temperature = data.readInt16BE(4)/256;
  var sensor_temp = temperature - 10;           // real temp = mesured temp-10 
...
EddystoneBeaconScanner.prototype.render = function(temperature, light, beacon, temp1) {

  return{
    data frame:{
      sensor_id: beacon.id,
      sensor_motion: temp1,              // if 1 => move else stands still
      sensor_light: 100*(light/25000),   // 25000 is the light level mesured by the Yunzi in standard daylight
      sensor_time: Date.now(),
      sensor_temp: temperature - 10   // real temp = mesured temp-10 
    }
  }
  console.log(sensor_temp);
  console.log(sensor_time);
  console.log(sensor_light);
  console.log(sensor_motion);
